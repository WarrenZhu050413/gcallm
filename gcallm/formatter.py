    ...
    if '✅' in response or 'Created' in response:
        ...
        for line in lines:
            ...
            if line.startswith('- **'):
                ...
                if 'Date & Time:' in content or '**Date & Time:**' in content:
                    datetime_text = re.sub(r'\*\*Date & Time:\*\*|\*\*', '', content).strip()
                    current_event['datetime'] = datetime_text
                elif 'Description:' in content:
                    desc_text = re.sub(r'\*\*Description:\*\*|\*\*', '', content).strip()
                    current_event['description'] = desc_text
                elif 'Event Link:' in content or 'Link:' in content:
                    # Extract URL
                    url_match = re.search(r'https://www\.google\.com/calendar[^\s\)]*', content)
                    if url_match:
            ...
            # Also check for standalone URLs
                ...
                url_match = re.search(r'https://www\.google\.com/calendar[^\s\)]*', line)
                if url_match:
                    current_event['link'] = url_match.group(0)
        ...
        if events:
            for event in events:
                ...
                # Add title
                if 'title' in event:
                    table.add_row("Event:", f"[bold green]{event['title']}[/bold green]")
            ...
            if '⚠️' in response or 'Note:' in response or 'conflict' in response.lower():
                ...
                for line in lines:
                    if '⚠️' in line or 'Note:' in line:
                        capture = True
                    if capture:
                        clean_line = line.strip().replace('⚠️', '').replace('**Note:**', '').strip()
                        if clean_line and not clean_line.startswith('✅'):
                            warning_lines.append(clean_line)

                if warning_lines:
                    warning_text = '\n'.join(warning_lines[:5])  # Limit to first 5 lines
                    console.print(Panel(
                        warning_text,
